            ...
            itemsTopics: [
                {
                    ...
                    subTitle: "<p>Driving uat donuts ground other across. Done inclusion pulling parking you not drawing-board want. Right horse corporate 30,000ft do ui.</p>"
                },
                {
                    title: "A Brief History of Web 📹",
                    content: {
...
export default function PrimarySection({ props }: { props: Props }) {
    ...
    return (
        <div class="container bg-black flex justify-center flex-col w-full py-6 rounded-3xl px-4">
            ...
            <span class="text-center text-lg text-[#A1A1AA]" dangerouslySetInnerHTML={{ __html: subTitle }} ></span>
            <div class="flex w-full flex-row mt-9 xl:gap-16">
                <div class="w-3/12 flex flex-col gap-3">
                    {topics.map((topic, indexTopic) => {
                        return (<>
                            <h3 class="text-accent text-2xl font-semibold">{topic.title}</h3>
                            {topic.itemsTopics.map((itemTopic, index) => (
                                <ButtonTab label={itemTopic.title} index={index.toString() + indexTopic.toString()} />
                            ))}
                        </>
                        )
                    })}
                ...
                <div class="w-3/4 flex flex-col py-8 lg:mx-6 xl:mx-14">
                    {topics.map((topic, indexTopic) => (
                        topic.itemsTopics.map((itemTopic, index) => (
                            <Article title={itemTopic.title} subTitle={itemTopic.subTitle} content={itemTopic.content} flagTime={itemTopic.flagTime} button={itemTopic.button} index={index.toString() + indexTopic.toString()} />
                        ))
                    ))}
                </div>
            </div>
